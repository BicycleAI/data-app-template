...
/** Every measure for every combination of the selected dimensions; click a header to sort, a row to select it. Waits on `core.dims`. */
export function Render({ spec, core, bind }: CoreProps) {
  ...
  const digest = rows.slice(0, 10).map((slice) => ({
    ...Object.fromEntries(dims.map((dim, index) => [dim, slice.values[index] ?? ''])),
    ...Object.fromEntries(spec.measures.map((measure) => [measure.id, slice.measures[measure.id]])),
  }))
  return (
