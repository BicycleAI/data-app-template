...
- **Spec**: `spec/dataapp-spec.v2.schema.json` — the contract: any model, any measures, any cuts, optional entity, optional analysis family. Examples in `spec/examples/` (a retail orders-health report on one model, three A/B-test apps on another).
...
node compose/cli.mjs compose spec/examples/checkout-test-report.json --out build/checkout-test-report
# upload build/checkout-test-report.zip with dataapp_upload_url / --upload-url, then dataapp_complete_upload + dataapp_publish
```
