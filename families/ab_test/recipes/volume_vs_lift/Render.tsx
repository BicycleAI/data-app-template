...
export function Render({ spec, data, bind }: RecipeProps) {
  ...
  const rows = useMemo(
    () =>
      (variant === undefined ? [] : data.segmentsAt(ui.dims, 2).filter((segment) => segment.variant === variant.name))
  ...
  return (
    <div className="bda-card kit-panel">
      <SectionHead title="Volume vs lift" right={<span className="bda-subtle">every two-dimension segment · hover a point</span>} />
